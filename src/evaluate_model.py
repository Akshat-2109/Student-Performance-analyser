...
def evaluate_all_models():
    ...
    if results:
        ...
        # ✅ Save updated results to model_comparison.json so dashboard stays in sync
        json_results = [
            {
                "model": r["name"],
                "MAE": round(r["MAE"], 4),
        ...
        with open(out_json, 'w') as f:
            json.dump({"best_model": best_model["name"], "results": json_results}, f, indent=2)
        print(f"\n✅ model_comparison.json updated → {out_json}")
