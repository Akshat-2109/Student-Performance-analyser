...
MODEL_JSON = os.path.join(ROOT, 'outputs', 'model_comparison.json')

parser = argparse.ArgumentParser()
parser.add_argument('--retrain', action='store_true', help='Force retrain model')
parser.add_argument('--port', type=int, default=5000, help='Port number (default 5000)')
# parse_known_args (not parse_args): when this module is imported by gunicorn
# (e.g. "gunicorn run:app"), sys.argv contains gunicorn's own flags like
# "-w 1 -b 0.0.0.0:$PORT run:app", which parse_args() would reject and crash
# on. parse_known_args() safely ignores anything it doesn't recognize.
...
# Step 2: Train model
if not os.path.exists(MODEL_JSON) or args.retrain:
...
from app.app import app  # exposed at module level so gunicorn can use "run:app"

if __name__ == '__main__':
    # Only start Flask's own dev server when run directly (python run.py).
    # When gunicorn imports this file (production/Render), it supplies its
    # own server and binds 0.0.0.0:$PORT itself — this block must NOT run
    # then, or it would start a second, conflicting server on 127.0.0.1.
