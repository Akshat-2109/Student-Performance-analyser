...
# Rate Limiter
# NOTE: keyed by (ip, endpoint) — NOT just ip. Previously every @rate_limit
# route shared a single per-IP bucket, so normal browsing (analytics, students,
# predict calls) silently ate into the /api/security endpoint's own budget and
# made the Security tab's "Refresh" button appear to randomly stop working
# once ~60 combined requests had been made in a minute. Each endpoint now gets
...
def is_rate_limited(ip: str, key: str) -> bool:
    now  = time.time()
    bucket = (ip, key)
    hits = [t for t in _RATE_STORE[bucket] if now - t < RATE_WINDOW]
    _RATE_STORE[bucket] = hits
    if len(hits) >= RATE_LIMIT:
        audit('RATE_LIMIT', f'Blocked after {len(hits)} requests on {key}', 'WARNING', ip)
        return True
    hits.append(now)
    return False
...
def rate_limit(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if is_rate_limited(ip, f.__name__):
