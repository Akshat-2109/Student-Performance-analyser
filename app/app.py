...
def api_predict():
    ...
    # Academic: normalized marks score (0-100), then weighted by attendance.
    # Formula:  marks_score = (PrevScores + Test×2 + Project - 70) / 130 × 100
    #           attendance_weight = Attendance / 100  (0.0 → 1.0)
    #           final = marks_score × attendance_weight
    # Rationale: A student who never attends (0%) cannot credibly earn academic
    #            marks — attendance is a prerequisite for academic credit.
    raw_academic   = float(clean['Previous_Scores']) + float(clean['Test_Score']) * 2 + float(clean['Project_Marks'])
    marks_score    = (raw_academic - 70) / (200 - 70) * 100          # 0-100
    att_weight     = attendance_val / 100                              # 0.0-1.0
    academic_score = round(marks_score * att_weight, 1)               # 0-100

    # Attendance: displayed as a 0-10 score (Attendance% / 10)
    attendance_score = round(attendance_val / 10, 1)

    # Study Habit: Hours_Studied max=40 → scaled to 0-10
